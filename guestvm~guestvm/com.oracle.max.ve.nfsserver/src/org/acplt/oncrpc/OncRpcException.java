...
/**
 * The class <code>OncRpcException</code> indicates ONC/RPC conditions that a
 * reasonable application might want to catch. We follow here the notation
 * established by the Java environment that exceptions can be caught while
...
public class OncRpcException extends Exception {
    ...
    /**
     * A buffer overflow occured with an encoding XDR stream. This happens if
     * you use UDP-based (datagram-based) XDR streams and you try to encode more
     * data than can fit into the sending buffers.
     */
    public static final int RPC_BUFFEROVERFLOW = 42;

    /**
     * A buffer underflow occured with an decoding XDR stream. This happens if
     * you try to decode more data than was sent by the other communication
     * partner.
     */
    public static final int RPC_BUFFERUNDERFLOW = 43;

    /**
     * Indicates that a server could not register a transport with the ONC/RPC
     * port mapper.
     */
    public static final int RPC_CANNOTREGISTER = 45;

    /**
     * The ONC/RPC server could not decode the arguments sent within the call
     * message.
     */
    ...
    /**
     * The ONC/RPC server encountered a system error and thus was not able to
     * carry out the requested remote function call successfully.
     */
     ...
     * {@link OncRpcProtocols#ONCRPC_TCP} and {@link OncRpcProtocols#ONCRPC_UDP}
     * are supported.
     */
     ...
     * message when waiting for a request or reply. Currently, only the decoding
     * methods of the classes {@link OncRpcCallMessage} and
     ...
     *         {@link #OncRpcException() created} with no error reason.
     */
