...
import org.acplt.oncrpc.OncRpcPortmapServices;
...
public class jportmap extends OncRpcServerStub implements OncRpcDispatchable {
     ...
     * supply it the transports to handle. Registration and deregistration is
     * not necessary and not possible.
     */
     ...
     * information and UDP and TCP-based transports, which will be bound later
     * to port 111. The constructor does not start the dispatcher loop.
     */
    public jportmap() throws OncRpcException, IOException {
        ...
        transports = new OncRpcServerTransport[] {
                                                  new OncRpcUdpServerTransport(
                                                                               this,
                                                                               PMAP_PORT,
                                                                               info,
                                                                               32768),
                                                  new OncRpcTcpServerTransport(
                                                                               this,
                                                                               PMAP_PORT,
                                                                               info,
                                                                               32768) };
        ...
        //
        servers.addElement(new OncRpcServerIdent(PMAP_PROGRAM, PMAP_VERSION,
        ...
        for (InetAddress local: locals) {
            System.err.println("Portmap local addr: " + local);
        }
    }
     ...
     * @param program
     *            the portmap's program number, 100000
     * @param version
        ...
        if (program == PMAP_PROGRAM) {
            if (version == PMAP_VERSION) {
                switch (procedure) {
                    ...
                    case OncRpcPortmapServices.PMAP_GETPORT: { // handle port
                        // query
                    ...
                    case OncRpcPortmapServices.PMAP_SET: { // handle port
                        // registration
                        //
                    ...
                    case OncRpcPortmapServices.PMAP_DUMP: { // list all
                        // registrations
                        call.retrieveCall(XdrVoid.XDR_VOID);
     ...
     * Note that a caller can not register the same (program, version, protocol)
