...
/**
 * The class <code>OncRpcEmbeddedPortmap</code> provides an embeddable portmap
 * service, which is automatically started in its own thread if the (operating)
...
public class OncRpcEmbeddedPortmap {
    ...
    /**
     * Extend the portmap service so that it automatically takes itself out of
     * service when the last ONC/RPC programs is deregistered.
     */
    ...
    class OncRpcEmbeddedPortmapThread extends Thread {
        ...
        /**
         * Construct a new embedded portmap service thread and associate it with
         * the portmap object to be used as the service. The service is not
         * started yet.
         */
        ...
        /**
         * Run the embedded portmap service thread, starting dispatching of all
         * portmap transports until we get the signal to shut down.
         */
        @Override
        public void run() {
            try {
    ...
    /**
     * Portmap object acting as embedded portmap service or <code>null</code> if
     * no embedded portmap service is necessary because the operating system
     * already supplies one or another portmapper is already running.
     */
    ...
    public OncRpcEmbeddedPortmap(int checkTimeout) throws OncRpcException,
        ...
        if (!isPortmapRunning(checkTimeout)) {
            ...
            embeddedPortmapThread = new OncRpcEmbeddedPortmapThread(
                                                                    embeddedPortmap);
     ...
     * not use this method except you need to force the embedded portmap service
