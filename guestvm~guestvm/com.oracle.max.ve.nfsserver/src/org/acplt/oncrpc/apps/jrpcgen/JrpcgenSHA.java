...
/**
 * This class implements the SHA-1 algorithm as described in "Federal
 * Information Processing Standards Publication 180-1: Specifications for the
...
public class JrpcgenSHA {
    ...
    public long getHash() {
        byte[] hash = getDigest();
        return (((long) hash[0]) & 0xFF) + ((((long) hash[1]) & 0xFF) << 8)
               + ((((long) hash[2]) & 0xFF) << 16)
               + ((((long) hash[3]) & 0xFF) << 24)
               + ((((long) hash[4]) & 0xFF) << 32)
               + ((((long) hash[5]) & 0xFF) << 40)
    ...
    /**
     * Process a single block. This is pretty much copied verbatim from "Federal
     * Information Processing Standards Publication 180-1: Specifications for
     * the Secure Hash Standard. April 17, 1995.", pp. 9, 10.
     */
