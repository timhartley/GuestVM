...
public class OncRpcClientAuthUnix extends OncRpcClientAuth {
     ...
     * optionally returned by an ONC/RPC server to be used on subsequent ONC/RPC
     * calls.
     */
     ...
     */
    @Override
    protected void xdrDecodeVerf(XdrDecodingStream xdr) throws OncRpcException,
                                                       IOException {
        //
        ...
        switch (xdr.xdrDecodeInt()) {
            case OncRpcAuthType.ONCRPC_AUTH_NONE:
                ...
                // Anything different from this is not kosher and an
                // authentication
     ...
     * verifier when sending an ONC/RPC call message. The <code>AUTH_UNIX</code>
     * authentication method only uses the credential but no verifier. If the
    ...
    protected void xdrEncodeCredVerf(XdrEncodingStream xdr)
        ...
        if (shorthandCred == null) {
            ...
            if ((gids.length > OncRpcAuthConstants.ONCRPC_MAX_GROUPS)
                || (machinename.length() > OncRpcAuthConstants.ONCRPC_MAX_MACHINE_NAME)) {
                throw (new OncRpcAuthenticationException(
                                                         OncRpcAuthStatus.ONCRPC_AUTH_FAILED));
            }
            xdr.xdrEncodeInt(OncRpcAuthType.ONCRPC_AUTH_UNIX);
            int len = 4 // length of stamp
                      + ((machinename.length() + 7) & ~3) // len string incl.
                      // len
                      + 4 // length of uid
                      + 4 // length of gid
                      + gids.length * 4 + 4 // length of vector of gids incl.
            // len
            ;
