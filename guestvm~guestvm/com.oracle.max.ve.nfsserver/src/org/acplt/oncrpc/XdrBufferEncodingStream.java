/*
 * $Header:
 * /cvsroot/remotetea/remotetea/src/org/acplt/oncrpc/XdrBufferEncodingStream
...
/**
 * The <code>XdrBufferEncodingStream</code> class provides a buffer-based XDR
 * stream.
 *
...
public class XdrBufferEncodingStream extends XdrEncodingStream {
    ...
    /**
     * The buffer which will receive the encoded information, before it is sent
     * via a datagram socket.
     */
    private byte[] buffer;
     ...
     *
     * @param buffer
     *            Buffer to store encoded information in.
     *
    ...
    public XdrBufferEncodingStream(byte[] buffer) {
        ...
        if ((buffer.length & 3) != 0) {
            throw (new IllegalArgumentException(
                                                "size of buffer must be a multiple of four"));
        }
        this.buffer = buffer;
        bufferIndex = 0;
    ...
    /**
     * Constructs a new <code>XdrBufferEncodingStream</code> with a buffer to
     * encode data into of the given size.
     *
     * @param bufferSize
    ...
    public XdrBufferEncodingStream(int bufferSize) {
        if ((bufferSize < 0) || (bufferSize & 3) != 0) {
            throw (new IllegalArgumentException(
                                                "size of buffer must be a multiple of four and must not be negative"));
        }
