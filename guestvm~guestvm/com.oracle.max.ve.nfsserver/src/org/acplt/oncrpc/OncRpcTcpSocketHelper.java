...
/**
 * Wraps JRE-specific networking code for TCP/IP-based client sockets. So much
 * for compile once, make it unuseable everywhere. Why could our great Sun
 * simply not get their socket class straight from the beginning? The BSD socket
 * API has been around since long enough and that real network applications need
 ...
 * especially their support for the Web. Sweet irony that it took them until
 ...
 * wrapper indicates the first JRE version supporting a particular feature:
 * <li>
...
public class OncRpcTcpSocketHelper {
    ...
    /**
     * The class <code>Connectiator</code> has a short and sometimes sad life,
     * as its only purpose is trying to connect to a TCP port at another host
     * machine.
     */
    private class Connectiator extends Thread {
        ...
        /**
         * Flag to indicate that the socket is not needed, as the caller timed
         * out.
         */
        private boolean hitTheBucket = false;

        /**
         * <code>IOException</code> caused by connection attempt, if any, or
         * <code>null</code>.
         */
         ...
         * connect to the given TCP port at the host specified. Note that we do
         * not try to establish the connection yet; this has to be done later
         * using the {@link #run} method.
         */
    ...
    /**
     * Constructor Socket() without any parameters or <code>null</code> if not
     * available in the class library of a particular JRE.
     */
    ...
    /**
     * Method Socket.getSendBufferSize or <code>null</code> if not available in
     * the class library of a particular JRE.
     */
    ...
    /**
     * Method Socket.setSendBufferSize or <code>null</code> if not available in
     * the class library of a particular JRE.
     */
    ...
    /**
     * Creates a stream socket helper but does not associates it with a real
     * stream socket object. You need to call {@link #connect} lateron for a
     * timeout-controlled connect.
     */
    ...
    public Socket connect(InetAddress address, int port, int timeout)
        ...
        if (methodConnect != null) {
            ...
            synchronized (connectiator) {
                ...
                //
                // We either could connect successfully, or the connect
                // operation
    ...
    public int getReceiveBufferSize() throws SocketException {
        if (methodGetReceiveBufferSize != null) {
            try {
                Object result = methodGetReceiveBufferSize.invoke(
                                                                  socket,
                                                                  (Object[]) null);
    ...
    public int getSendBufferSize() throws SocketException {
        if (methodGetSendBufferSize != null) {
            try {
                Object result = methodGetSendBufferSize.invoke(socket,
                                                               (Object[]) null);
                if (result instanceof Integer) {
                    return ((Integer) result).intValue();
                }
     ...
     *             if <code>size</code> is 0 or negative.
     */
    public void setReceiveBufferSize(int size) throws SocketException {
        if (methodSetSendBufferSize != null) {
            try {
                methodSetReceiveBufferSize.invoke(
                                                  socket,
    ...
    /**
     * Sets the socket's send buffer size as a hint to the underlying transport
    ...
    public void setSendBufferSize(int size) throws SocketException {
        if (methodSetSendBufferSize != null) {
            try {
                methodSetSendBufferSize.invoke(
                                               socket,
    ...
    /**
     * Looks up methods of class Socket whether they are supported by the class
     * libraries of the JRE we are currently executing on.
     */
    protected void inspectSocketClassMethods() {
        Class socketClass = Socket.class;
        // JRE 1.2 specific stuff
        try {
            methodSetSendBufferSize = socketClass.getMethod(
                                                            "setSendBufferSize",
                                                            new Class[] { int.class });
            methodGetSendBufferSize = socketClass.getMethod(
                                                            "getSendBufferSize",
                                                            (Class[]) null);
        } catch (Exception e) {
        }
        try {
            methodSetReceiveBufferSize = socketClass.getMethod(
                                                               "setReceiveBufferSize",
                                                               new Class[] { int.class });
            methodGetReceiveBufferSize = socketClass.getMethod(
        ...
        try {
            ...
            methodConnect = socketClass.getMethod(
                                                  "connect",
                                                  new Class[] {
                                                               InetAddress.class,
                                                               int.class });
