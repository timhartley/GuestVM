...
public class XdrTcpDecodingStream extends XdrDecodingStream {
    ...
    /**
     * Index of the last four byte word in the buffer, which has been read in
     * from the datagram socket.
     */
    ...
    /**
     * Flag indicating that we've read the last fragment and thus reached the
     * end of the record.
     */
     ...
     * @param bufferSize
     *            Size of packet buffer for storing received XDR data.
     */
    public XdrTcpDecodingStream(Socket streamingSocket, int bufferSize)
                                                                       throws IOException {
     ...
     */
    @Override
    public byte[] xdrDecodeOpaque(int length) throws OncRpcException,
                                             IOException {
        int padding = (4 - (length & 3)) & 3;
    ...
    private void fill() throws OncRpcException, IOException {
        ...
        if (fragmentLength <= 0) {
            ...
            if ((fragmentLength == 0) && !lastFragment) {
                throw (new IOException(
                                       "empty ONC/RPC XDR fragment which is not a trailing fragment"));
            }
        }
        //
