 ...
 * which can talk to the portmapper on a given host using the famous UDP/IP
 * datagram-oriented internet protocol. In addition, it is also possible to
 ...
 * <b>not</b> supported as the transport-independent ONC/RPC implementation is
 * not that widely in use due to the brain-damaged design of XTI. If you should
 ...
 * happens to be the program number of the <a
 ...
 * secondm parameter specifies the program's version number, and the third
 * parameter specifies the IP protocol to use when issueing ONC/RPC calls.
 ...
 * server waits for incoming ONC/RPC calls. If the ONC/RPC program is not
 * registered with the particular ONC/RPC portmapper, an
...
public class OncRpcPortmapClient {
    ...
    /**
     * The particular transport-specific ONC/RPC client object used for talking
     * to the portmapper.
     */
     ...
     */
    public OncRpcPortmapClient(InetAddress host) throws OncRpcException,
                                                IOException {
     ...
     *            Host where to contact the portmapper.
     * @param protocol
     *            Protocol to use for contacting the portmapper. This can be
     ...
     * @throws OncRpcProgramNotRegisteredException
     *             if the requested program is not available.
     */
    public int getPort(int program, int version, int protocol)
        ...
        //
        OncRpcServerIdent params = new OncRpcServerIdent(program, version,
                                                         protocol, 0);
        ...
        //
        try {
            portmapClient.call(OncRpcPortmapServices.PMAP_GETPORT, params,
                               result);
        } catch (OncRpcException e) {
    ...
    public OncRpcServerIdent[] listServers() throws OncRpcException {
        ...
        //
        try {
            portmapClient.call(OncRpcPortmapServices.PMAP_DUMP,
                               XdrVoid.XDR_VOID, result);
        } catch (OncRpcException e) {
            throw (new OncRpcException(OncRpcException.RPC_PMAPFAILURE));
        }
        //
        ...
        //
        OncRpcServerIdent[] info = new OncRpcServerIdent[result.servers.size()];
     ...
     * @param protocol
     *            The protocol spoken by the ONC/RPC server. Can be one of the
