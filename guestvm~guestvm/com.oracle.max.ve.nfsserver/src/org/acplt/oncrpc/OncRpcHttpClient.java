 ...
 * parsing XML is still too large an overhead to accept. Despite cynics pointing
 * out that the Internet is already that slow so that XML overhead will not be
...
public class OncRpcHttpClient extends OncRpcClient {
    ...
    /**
     * Path of cgi program redirecting ONC/RPC calls to the appropriate ONC/RPC
     * servers.
     */
    private String cgiHandlerPath;

    /**
     * DNS name of host where to contact HTTP server. Note that we can not use
     * an <code>InetAddress</code> here as we have to support virtual hosting,
     * where several DNS names share the same IP address.
     */
    private String hostname;

    /**
     * The HTTP client responsible for handling the HTTP connection. It is a
     * classic example of delegation, isn't it?
     */
    ...
    /**
     * Transport protocol to be used by the other end of the tunnel to contact
     * the ONC/RPC server.
     */
    private int oncrpcProtocol;

    /**
     * XDR decoding stream used when receiving replies via an HTTP tunnel from
     * an ONC/RPC server.
     */
    private XdrHttpDecodingStream receivingXdr;

    /**
     * FIXME: use the right encoding stream! XDR encoding stream used for
     * sending requests via UDP/IP to an ONC/RPC server.
     */
     ...
     * @param httpPort
     *            The port number where the HTTP server is to be contacted.
     * @param cgiHandlerPath
     *            The path to the CGI Handler which will redirect ONC/RPC calls
     *            to the particular ONC/RPC servers.
     * @param oncrpcHostname
     *            The DNS name of the ONC/RPC server to contact.
     * @param program
     ...
     */
    public OncRpcHttpClient(String hostname, int httpPort,
                            String cgiHandlerPath, String oncrpcHostname,
                            int program, int version, int port, int protocol)
                                                                             throws OncRpcException,
                                                                             IOException {
        //
    ...
    public OncRpcHttpClient(String hostname, String cgiHandlerPath,
                            String oncrpcHostname, int program, int version,
                            int port) throws OncRpcException, IOException {
        this(hostname, HttpClientConnection.HTTP_DEFAULTPORT, cgiHandlerPath,
             oncrpcHostname, program, version, port, OncRpcProtocols.ONCRPC_TCP);
    }
    ...
    public OncRpcHttpClient(String hostname, String cgiHandlerPath,
                            ...
                            int port, int protocol) throws OncRpcException,
                                                   IOException {
        this(hostname, HttpClientConnection.HTTP_DEFAULTPORT, cgiHandlerPath,
             oncrpcHostname, program, version, port, protocol);
    }
    ...
    public synchronized void call(int procedureNumber, int versionNumber,
        ...
        Refresh: for (int refreshesLeft = 1; refreshesLeft >= 0; --refreshesLeft) {
            ...
            prefix.append(" ");
            prefix.append(HttpTunnelConstants.TUNNEL_PROTO_ID);
            prefix.append("\r\n");
            //
            ...
            int realLength = prefix.length() + ((contentLength + 2) / 3) * 4
                             + lineCount * 2;
            // FIXME
            ...
            //
            try {
                httpClient.beginPostRequest(cgiHandlerPath, "text/plain",
                                            realLength);
                //
                ...
                while (contentLength >= HttpTunnelConstants.BYTES_PER_LINE) {
                    if (offset >= (HttpTunnelConstants.ENCODED_BYTES_PER_LINE_CRLF * HttpTunnelConstants.LINES_PER_BLOCK)) {
