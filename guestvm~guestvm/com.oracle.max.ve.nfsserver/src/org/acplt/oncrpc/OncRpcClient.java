/*
 * $Header:
 * /cvsroot/remotetea/remotetea/src/org/acplt/oncrpc/OncRpcClient.java,v 1.3
 * 2003/08/14 13:48:33 haraldalbrecht Exp $
 *
 ...
 * functionality, like port resolving, if no port was specified for the ONC/RPC
 * server to contact. This class also provides the method skeleton, for instance
 * for executing procedure calls.
 *
 * <p>
 * In order to communicate with an ONC/RPC server, you need to create an ONC/RPC
 * client, represented by classes derived from <code>OncRpcClient</code>. The
 * most generic way to generate an ONC/RPC client is as follows: use
 * {@link #newOncRpcClient(InetAddress, int, int, int) newOncRpcClient(...)} and
 * specify:
 *
 * <ul>
 ...
 * <li>and finally the IP protocol to use when talking to the server. This can
 * be either {@link OncRpcProtocols#ONCRPC_UDP} or
 ...
 * OncRpcClient client;
 * try {
 *     client = OncRpcClient.newOncRpcClient(InetAddress.getByName(&quot;localhost&quot;),
 *                                           0x49678, 1,
 ...
 *
 * <p>
 * This code snippet also shows exception handling. The most common error you'll
 * see is probably an {@link OncRpcProgramNotRegisteredException}, in case no
 * such program number is currently registered at the specified host. An
 * {@link OncRpcProgramNotRegisteredException} is a subclass of
 * {@link OncRpcException} with a detail of
 * {@link OncRpcException#RPC_PROGNOTREGISTERED}. In case no ONC/RPC portmapper
 * is available at the specified host, you'll get an
 * {@link OncRpcTimeoutException} instead (which is again a subclass of
 * <code>OncRpcException</code> with a detail of
 * {@link OncRpcException#RPC_TIMEDOUT}). You might also get an IOException when
 * using TCP/IP and the server can not be contacted because it does not accept
 * new connections.
 *
 * <p>
 * Instead of calling {@link #newOncRpcClient(InetAddress, int, int, int)
 * OncRpcClient.newOncRpcClient(...)} you can also directly create objects of
 * classes {@link OncRpcTcpClient} and {@link OncRpcUdpClient} if you know at
 * compile time which kind of IP protocol you will use.
 *
 * <p>
 * With a client proxy in your hands, you can now issue ONC/RPC calls. As a
 * really, really simple example -- did I say "simple example"? -- we start with
 * the famous ONC/RPC ping call. This call sends no parameters and expects no
 * return from an ONC/RPC server. It is just used to check whether a server is
 * still responsive.
 *
 * <pre>
 ...
 *
 * <p>
 * By definition, the ONC/RPC ping call has program number 0 and expects no
 * parameters and replies with no result. Thus we just specify an empty
 ...
 *
 * <p>
 * For more complex and sometimes more useful ONC/RPC calls, you will need to
 * write appropriate ONC/RPC parameter and reply classes. Unfortunately, at this
 * time there's no compiler available to compile <code>.x</code> files into
 * appropriate Java classes. Well -- surely a lot of people will now associate
 * completely wrong things with "x files", but in our case there's no new age or
 * whatever mumbo jumbo involved, but definitions of XDR data structures
 * instead.
 *
 * For the next example, let's pretend our server provides the answer to all
 * questions when called with procedure number 42. Let's also pretend that this
 * ONC/RPC call expects a question in form of a string and returns the answer as
 * an integer. So we need to define two classes, one for the call's parameters
 * and one for the reply. But let us first examine the class containing a call's
 * parameters:
 *
 * <pre>
 ...
 *
 * <p>
 * The <code>Parameters</code> class implements {@link XdrAble}, so instances of
 * it can be sent and received over the network using Sun's XDR protocol. What
 * exactly is sent over the wire is up to the two methods
 * {@link XdrAble#xdrEncode xdrEncode(...)} and {@link XdrAble#xdrDecode
 * xdrDecode(...)}. The <code>xdrEncode</code> method is responsible to "encode"
 * the data to be sent over the network. On the other side,
 * <code>xdrDecode</code> is responsible to restore an object's state from the
 * data received over the network. In our example, these methods either send or
 * receive a string.
 *
 * <p>
 * The class defining the reply of our the-answer-to-all-questions ONC/RPC call
 ...
 *
 * <p>
 * Finally, to ask a question, you need to create the parameter object and fill
 * it with the parameters to be sent. Then create the object later receiving the
 * reply. Finally issue the ONC/RPC call:
 *
 * <pre>
 ...
 *
 * <p>
 * {@link OncRpcClientAuth Authentication} can be done as follows: just create
 * an authentication object and hand it over to the ONC/RPC client object.
 *
 * <pre>
 * OncRpcClientAuth auth = new OncRpcClientAuthUnix(&quot;marvin@ford.prefect&quot;, 42,
 *                                                  1001, new int[0]);
 * client.setAuth(auth);
 * </pre>
 *
 ...
 * </pre>
 *
 * In the example above, three calls are batched in a row and only be sent all
 * together with the third call. Note that batched calls must not expect
...
public abstract class OncRpcClient {
     ...
     * @param protocol
     *            {@link OncRpcProtocols Protocol} to be used for ONC/RPC calls.
     *
     ...
     */
    public static OncRpcClient newOncRpcClient(InetAddress host, int program,
     ...
     *            Version number of the desired ONC/RPC server.
     * @param port
     *            Port number of the ONC/RPC server. Specifiy <code>0</code> if
     *            this is not known and the portmap process located at host
     *            should be contacted to find out the port.
     * @param protocol
     *            {@link OncRpcProtocols Protocol} to be used for ONC/RPC calls.
     *
     * @throws OncRpcException
     ...
     * will throw an exception if no answer from the ONC/RPC server is received
    ...
    /**
     * The message id (also sometimes known as "transaction id") used for the
     * next call message.
     */
    protected int xid;

    /**
     * Constructs an <code>OncRpcClient</code> object (the generic part). If no
     * port number is given (that is, <code>port</code> is <code>0</code>), then
     * a port lookup using the portmapper at <code>host</code> is done.
     *
     * @param host
     *            Host address where the desired ONC/RPC server resides.
     * @param program
     ...
     * @param protocol
     *            {@link OncRpcProtocols Protocol} to be used for ONC/RPC calls.
     *            This information is necessary, so port lookups through the
     *            portmapper can be done.
     *
     * @throws OncRpcException
     *             if an ONC/RPC error occurs.
     * @throws IOException
     *             if an I/O error occurs.
     */
    protected OncRpcClient(InetAddress host, int program, int version,
                           int port, int protocol) throws OncRpcException,
                                                  IOException {
        //
        // Set up the basics...
        //
     ...
     * @param parameters
     *            The parameters of the procedure to call, contained in an
     ...
     *
     * <p>
     * The <code>OncRpcUdpClient</code> uses a similar timeout scheme as the
     * genuine Sun C implementation of ONC/RPC: it starts with a timeout of one
     * second when waiting for a reply. If no reply is received within this time
     * frame, the client doubles the timeout, sends a new request and then waits
     * again for a reply. In every case the client will wait no longer than the
     * total timeout set through the {@link #setTimeout(int)} method.
     *
     * @param procedureNumber
     *            Procedure number of the procedure to call.
     * @param params
     ...
     *             if an ONC/RPC error occurs.
     */
    public synchronized void call(int procedureNumber, XdrAble params,
                                  XdrAble result) throws OncRpcException {
        //
    ...
    /**
     * Set the timout for remote procedure calls to wait for an answer from the
     * ONC/RPC server. If the timeout expires,
     ...
     * indicated batched calls, for which no reply message is expected.
     *
     * @param milliseconds
