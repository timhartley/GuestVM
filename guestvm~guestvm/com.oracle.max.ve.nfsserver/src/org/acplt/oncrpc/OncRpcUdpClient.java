/*
 * $Header:
 * /cvsroot/remotetea/remotetea/src/org/acplt/oncrpc/OncRpcUdpClient.java,v 1.6
 * 2007/05/29 18:48:27 haraldalbrecht Exp $
 *
 ...
 * the datagram-oriented protocol UDP/IP.
 *
...
public class OncRpcUdpClient extends OncRpcClient {
     ...
     * server does not answer fast enough. The default retransmission timeout is
     * identical to the overall timeout for ONC/RPC calls (thus UDP/IP-based
    ...
    /**
     * Constructs a new <code>OncRpcUdpClient</code> object, which connects to
     * the ONC/RPC server at <code>host</code> for calling remote procedures of
     * the given { program, version }.
     *
     * <p>
     * Note that the construction of an <code>OncRpcUdpProtocolClient</code>
     ...
     * @param port
     *            The port number where the ONC/RPC server can be contacted. If
     ...
     */
    public OncRpcUdpClient(InetAddress host, int program, int version, int port)
                                                                                throws OncRpcException,
                                                                                IOException {
     ...
     *            number.
     * @param bufferSize
     *            The buffer size used for sending and receiving UDP datagrams.
     *
     * @throws OncRpcException
     *             if an ONC/RPC error occurs.
     * @throws IOException
     *             if an I/O error occurs.
     */
    public OncRpcUdpClient(InetAddress host, int program, int version,
                           int port, int bufferSize) throws OncRpcException,
                                                    IOException {
        //
     ...
     * broadcast address when creating a <code>OncRpcUdpClient</code>. For every
     * reply received, an event containing the reply is sent to the
     ...
     * <code>broadcastCall</code> will only send the ONC/RPC call once. It will
     * then wait for answers until the timeout as set by
     * {@link #setTimeout(int)} expires without resending the reply.
     *
     * <p>
     * Note that you might experience unwanted results when using authentication
     * types other than {@link OncRpcClientAuthNone}, causing messed up
     * authentication protocol handling objects. This depends on the type of
     * authentication used. For <code>AUTH_UNIX</code> nothing bad happens as
     * long as none of the servers replies with a shorthand verifier. If it
     * does, then this shorthand will be used on all subsequent ONC/RPC calls,
     * something you probably do not want at all.
     *
     ...
     * @param result
     *            The object receiving the result of the procedure call. Note
     *            that this object is reused to deserialize all incomming
     *            replies one after another.
     * @param listener
     *            Listener which will get an {@link OncRpcBroadcastEvent} for
     *            every reply received.
     *
     * @throws OncRpcException
     *             if an ONC/RPC error occurs.
     */
    public synchronized void broadcastCall(int procedureNumber, XdrAble params,
                                           XdrAble result,
    ...
    public synchronized void call(int procedureNumber, int versionNumber,
                                  XdrAble params, XdrAble result)
                                                                 throws OncRpcException {
        Refresh: for (int refreshesLeft = 1; refreshesLeft >= 0; --refreshesLeft) {
            //
            // First, build the ONC/RPC call header. Then put the sending
            // stream into a known state and encode the parameters to be
            ...
            // and so on, until we have reached the timeout for the call in
            // total.
            // Note that this setting only applies if exponential back-off
            // retransmission has been selected. Per default we do not
            // retransmit
                ...
                for (;;) {
                    try {
                        ...
                        if (currentTimeout > resendTimeout) {
                            ...
                            // have a finite timeout, albeit maybe an
                            // infinitesimal
                            // finite one.
                            currentTimeout = 1;
                        }
                        ...
                        // address we've sent the ONC/RPC call to. Otherwise
                        // throw
                        // away the datagram packet containing the reply and
                        // start
                        ...
                        if (host.equals(receivingXdr.getSenderAddress())) {
                            ...
                            // from the server and this verifier was invalid,
                            // broken
                            // or tampered with, we will get an
                            // OncRpcAuthenticationException right here, which
                            // will
                            // propagate up to the caller. If the server
                            // reported
                            ...
                            try {
                                ...
                                // ** SF bug #1262106 **
                                //
                                // We ran into some sort of trouble. Usually
                                // this will have
                                // been a buffer underflow. Whatever, end the
                                // decoding process
                                // and ensure this way that the next call has a
                                // chance to start
                                // from a clean state.
                                //
                                receivingXdr.endDecoding();
                            ...
                            //
                            // Only deserialize the result, if the reply matches
                            // the call
                            // and if the reply signals a successful call. In
                            // case of an
                            // unsuccessful call (which mathes our call
                            // nevertheless) throw
                            // an exception.
                            //
                            if (replyHeader.messageId == callHeader.messageId) {
                                if (!replyHeader.successfullyAccepted()) {
                                    ...
                                    // problem. In this case first try to
                                    // refresh the
                                    // credentials.
                                    //
                                    if ((refreshesLeft > 0)
                                        && (replyHeader.replyStatus == OncRpcReplyStatus.ONCRPC_MSG_DENIED)
                                        && (replyHeader.rejectStatus == OncRpcRejectStatus.ONCRPC_AUTH_ERROR)
                                        && (auth != null)
                                        && auth.canRefreshCred()) {
                                        //
                                ...
                                //
                                // The reply header is okay and the call had
                                // been
                                ...
                                try {
                                    ...
                                    // We ran into some sort of trouble. Usually
                                    // this will have
                                    // been a buffer underflow. Whatever, end
                                    // the decoding process
                                    // and ensure this way that the next call
                                ...
                                //
                                // Free pending resources of buffer and exit the
                                // call loop,
                                // returning the reply to the caller through the
                                // result
                                // object.
                                //
                                receivingXdr.endDecoding();
                                ...
                                // old UDP datagram which just popped up from
                                // the
                                ...
                                // harmful"...
                                //
                                // [Nothing to do here, just wait for the next
                                // datagram]
                            }
                        } else {
                            //
                            // IP address of received UDP datagram is not the
                            // same
                        ...
                        //
                        // Note that we only catch timeouts here, but no other
                        // exceptions. Those others will go up further until
                        // someone
                        // catches them. The timeouts are caught, so they can do
                        // no
                        // damage but instead we start another round of sending
                        // a
                        // request and waiting for a reply. Reminds me of NASA
                        // and
                        // their "Mars Polar Lander"...
                        //
                        // Note that we need to leave the inner waiting loop
                        // here,
            ...
            // the indecent language used in the previous sentence, this
            // software
    ...
    /**
     * Retrieve the current {@link OncRpcUdpRetransmissionMode retransmission
     * mode} set for retransmission of lost ONC/RPC calls.
     *
    ...
    /**
     * Set the {@link OncRpcUdpRetransmissionMode retransmission mode} for lost
     * remote procedure calls. The default retransmission mode is
    ...
    /**
     * Set the retransmission timout for remote procedure calls to wait for an
     * answer from the ONC/RPC server before resending the call. The default
     * retransmission timeout is the 30 seconds. The retransmission timeout must
     * be > 0. To disable retransmission of lost calls, set the retransmission
     * timeout to be the same value as the timeout.
     *
     * @param milliseconds
