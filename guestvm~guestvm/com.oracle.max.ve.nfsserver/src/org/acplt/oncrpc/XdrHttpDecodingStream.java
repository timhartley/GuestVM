...
public class XdrHttpDecodingStream extends XdrDecodingStream {
    ...
    public XdrHttpDecodingStream(HttpClientConnection httpClient) {
        ...
        int asciiBufferSize = lines
                              * HttpTunnelConstants.ENCODED_BYTES_PER_LINE_CRLF;
    ...
    private void fill() throws OncRpcException, IOException {
        ...
        try {
            ...
            if (remaining < 0) {
                charsRead = httpClient.readContentBytes(asciiBuffer, 0,
                                                        asciiBufferSize);
            } else {
                charsRead = httpClient.readContentBytes(asciiBuffer, 0,
                                                        remaining);
        ...
        while (charsRead > 0) {
            ...
            // and thus skip them.
            toDecode = charsRead >= HttpTunnelConstants.ENCODED_BYTES_PER_LINE ? HttpTunnelConstants.ENCODED_BYTES_PER_LINE
                                                                              : (charsRead - 2);
            decoded += Base64.decode(asciiBuffer, encoded, toDecode, buffer,
                                     decoded);
            //
            // FIXME: check CRLF
            //
