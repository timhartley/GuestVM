...
/**
 * The abstract <code>OncRpcServerStub</code> class is the base class to build
...
public abstract class OncRpcServerStub {
    ...
    /**
     * Notification flag for signalling the server to stop processing incomming
     * remote procedure calls and to shut down.
     */
    protected Object shutdownSignal = new Object();

    /**
     * Close all transports listed in a set of server transports. Only by
     ...
     * @throws IOException
     *             if a severe network I/O error occurs in the server from which
     *             it can not recover (like severe exceptions thrown when
     *             waiting for now connections on a server socket).
     */
     ...
     * transports. To end processing and to shut the server down signal the
