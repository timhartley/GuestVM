/*
 * $Header:
 * /cvsroot/remotetea/remotetea/src/org/acplt/oncrpc/server/OncRpcServerTransport
 * .java,v 1.3 2003/08/14 13:47:04 haraldalbrecht Exp $
 *
...
/**
 * Instances of class <code>OncRpcServerTransport</code> encapsulate XDR streams
 * of ONC/RPC servers. Using server transports, ONC/RPC calls are received and
 * the corresponding replies are later sent back after handling.
 *
 * <p>
 * Note that the server-specific dispatcher handling requests (done through
 * {@link OncRpcDispatchable} will only directly deal with
 * {@link OncRpcCallInformation} objects. These call information objects
 * reference OncRpcServerTransport object, but the server programmer typically
 * will never touch them, as the call information object already contains all
 * necessary information about a call, so replies can be sent back (and this is
 * definetely a sentence containing too many words).
 *
...
public abstract class OncRpcServerTransport {
     ...
     *
     * <p>
     * We do not create any XDR streams here, as it is the responsibility of
     * derived classes to create appropriate XDR stream objects for the
     ...
     *
     * <p>
     * Derived classes can choose between different behaviour for shuting down
     * the associated transport handler threads:
     * <ul>
     ...
     *
     * <p>
     * Note that you have to supply an implementation for this abstract method
     * in derived classes. Your implementation needs to create a new thread to
     * wait for incoming requests. The method has to return immediately for the
     * calling thread.
     */
    public abstract void listen();

    /**
     * Register the port where this server transport waits for incoming requests
     * with the ONC/RPC portmapper.
     *
     * <p>
     * The contract of this method is, that derived classes implement the
     * appropriate communication with the portmapper, so the transport is
     * registered only for the protocol supported by a particular kind of server
     * transport.
     *
     ...
     *
     * <p>
     * Note that due to the way Sun decided to implement its ONC/RPC portmapper
     * process, deregistering one server transports causes all entries for the
     * same program and version to be removed, regardless of the protocol
     * (UDP/IP or TCP/IP) used. Sigh.
     *
     * @throws OncRpcException
     *             with a reason of {@link OncRpcException#RPC_FAILED
     *             OncRpcException.RPC_FAILED} if the portmapper could not be
     *             contacted successfully. Note that it is not considered an
     *             error to remove a non-existing entry from the portmapper.
     */
     ...
     *
     * <p>
     * An appropriate implementation has to be provided in derived classes as it
     * is dependent on the type of transport (whether UDP/IP or TCP/IP) used.
     *
     * @param callInfo
     ...
     *             the network, etc.
     */
    protected abstract void retrieveCall(XdrAble call) throws OncRpcException,
                                                      IOException;
