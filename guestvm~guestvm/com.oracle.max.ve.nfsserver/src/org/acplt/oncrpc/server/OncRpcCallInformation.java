...
import org.acplt.oncrpc.OncRpcAcceptStatus;
import org.acplt.oncrpc.OncRpcAuthStatus;
import org.acplt.oncrpc.OncRpcCallMessage;
import org.acplt.oncrpc.OncRpcException;
import org.acplt.oncrpc.OncRpcRejectStatus;
import org.acplt.oncrpc.OncRpcReplyMessage;
import org.acplt.oncrpc.OncRpcReplyStatus;
import org.acplt.oncrpc.XdrAble;
import org.acplt.oncrpc.XdrDecodingStream;
import org.acplt.oncrpc.XdrEncodingStream;

/**
 * Objects of class <code>OncRpcCallInformation</code> contain information about
 * individual ONC/RPC calls. They are given to ONC/RPC
 * {@link OncRpcDispatchable call dispatchers}, so they can send back the reply
 * to the appropriate caller, etc. Use only this call info objects to retrieve
 * call parameters and send back replies as in the future UDP/IP-based
 ...
 * <ul>
 * <li>Use {@link #retrieveCall(XdrAble)} to retrieve the parameters of the call
 * and deserialize it into a paramter object.
 * <li>Use {@link #reply(XdrAble)} to send back the reply by serializing a
 ...
 * <ul>
 * <li>Use {@link #getXdrDecodingStream} to get a reference to the XDR stream
 * from which you can deserialize the call's parameter.
 * <li>When you are finished deserializing, call {@link #endDecoding}.
 * <li>To send back the reply/result, call
 * {@link #beginEncoding(OncRpcServerReplyMessage)}. Using the XDR stream
 * returned by {@link #getXdrEncodingStream} serialize the reply/result. Finally
...
public class OncRpcCallInformation {
    ...
    /**
     * Contains the call message header from ONC/RPC identifying this particular
     * call.
     */
    public OncRpcServerCallMessage callMessage = new OncRpcServerCallMessage();

    /**
     * Internet address of the peer from which we received an ONC/RPC call or
     * whom we intend to call.
     */
    public InetAddress peerAddress = null;

    /**
     * Port number of the peer from which we received an ONC/RPC call or whom we
     * intend to call.
     */
     ...
     * and to which we serialize the ONC/RPC reply. Never mess with this member
     * or you might break all future extensions horribly -- but this warning
     * probably only stimulates you...
     */
    protected OncRpcServerTransport transport;

    /**
     * Create an <code>OncRpcCallInformation</code> object and associate it with
     * a ONC/RPC server transport. Typically, <code>OncRpcCallInformation</code>
     * objects are created by transports once before handling incoming calls
     ...
     * sending you can serialize the result/reply. The stream to use for
    ...
    /**
     * Begins the sending phase for ONC/RPC replies. After beginning sending you
     * can serialize the reply/result (but only if the call was accepted, see
     * {@link org.acplt.oncrpc.OncRpcReplyMessage} for details). The stream to
    ...
    /**
     * Send back an ONC/RPC reply to the caller who sent in this call. This is a
