...
public class OncRpcTcpServerTransport extends OncRpcServerTransport {
    ...
    private class TransportList {
        ...
        private class Node {
            ...
            /**
             * Next item node (in other words: next open transport) in the list.
             * This will never be <code>null</code> for the first item, but
             * instead reference the last item. Thus, the list is circular.
             */
            Node next;

            /**
             * Previous item node (in other words: previous open transport) in
             * the list. This will never be <code>null</code> for the last item,
             * but instead reference the first item. Thus, the list is circular.
             */
            Node prev;

            /**
             * Create a new instance of a node object and let it reference an
             * open transport. The creator of this object is then responsible
             * for adding this node to the circular list itself.
             */
        ...
        /**
         * Head node for list of open transports which does not represent an
         * open transport but instead excuses us of dealing with all the special
         * cases of real nodes at the begin or end of the list.
         */
        ...
        /**
         * Add new transport to list of open transports. The new transport is
         * always added immediately after the head of the linked list.
         */
    ...
    /**
     * Encoding to use when deserializing strings or <code>null</code> if the
     ...
     * @param bufferSize
     *            Size of buffer used when receiving and sending chunks of XDR
     *            fragments over TCP/IP. The fragments built up to form ONC/RPC
     *            call and reply messages.
     */
    public OncRpcTcpServerTransport(
     ...
     *            call and reply messages.
     */
    public OncRpcTcpServerTransport(OncRpcDispatchable dispatcher, int port,
     ...
     * all server transports handling the individual TCP/IP connections will
     * also be closed. The handler threads will therefore either terminate
     * directly or when they try to sent back replies.
     */
    @Override
    public void close() {
        if (socket != null) {
        ...
        synchronized (openTransports) {
            while (openTransports.size() > 0) {
                OncRpcTcpConnectionServerTransport transport = (OncRpcTcpConnectionServerTransport) openTransports.removeFirst();
                transport.close();
            }
        }
    }
    ...
    public void setTransmissionTimeout(int milliseconds) {
        if (milliseconds <= 0) {
            throw (new IllegalArgumentException(
                                                "transmission timeout must be > 0"));
        }
    ...
    protected void beginEncoding(OncRpcCallInformation callInfo,
                                 OncRpcServerReplyMessage state)
                                                                ...
                                                                IOException {
        throw (new Error(
                         "OncRpcTcpServerTransport.beginEncoding() is abstract "
                                 + "and can not be called."));
    }
    ...
    protected XdrDecodingStream getXdrDecodingStream() {
        throw (new Error(
                         "OncRpcTcpServerTransport.getXdrDecodingStream() is abstract "
                                 + "and can not be called."));
    }
    ...
    protected XdrEncodingStream getXdrEncodingStream() {
        throw (new Error(
                         "OncRpcTcpServerTransport.getXdrEncodingStream() is abstract "
                                 + "and can not be called."));
    }
