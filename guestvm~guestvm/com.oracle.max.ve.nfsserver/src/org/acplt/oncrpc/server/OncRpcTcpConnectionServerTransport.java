...
public class OncRpcTcpConnectionServerTransport extends OncRpcServerTransport {
    ...
    /**
     * Reference to the TCP/IP transport which created us to handle a new
     * ONC/RPC connection.
     */
    ...
    /**
     * Indicates that <code>BeginEncoding</code> has been called for the sending
     * XDR stream, so in face of exceptions we can not send an error reply to
     * the client but only drop the connection.
     */
    private boolean pendingEncoding = false;

    /**
     * XDR decoding stream used when receiving requests via TCP/IP from ONC/RPC
     * clients.
     */
     ...
     * @param socket
     *            TCP/IP-based socket of new connection.
     * @param program
     ...
     * @param parent
     *            Parent server transport which created us.
     * @param transmissionTimeout
     *            Inherited transmission timeout.
     */
    public OncRpcTcpConnectionServerTransport(OncRpcDispatchable dispatcher,
                                              Socket socket, int program,
                                              int version, int bufferSize,
     ...
     *            Inherited transmission timeout.
     */
    public OncRpcTcpConnectionServerTransport(
     ...
     * thread will therefore either terminate directly or when it tries to sent
     * back replies.
     */
    @Override
    public void close() {
    ...
    /**
     * Creates a new thread and uses this thread to handle the new connection to
    ...
    /**
     * Finalize object by making sure that we're removed from the list of open
     * transports which our parent transport maintains.
     */
    @Override
