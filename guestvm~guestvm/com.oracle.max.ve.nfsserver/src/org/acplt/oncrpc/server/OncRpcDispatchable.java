/*
 * $Header:
 * /cvsroot/remotetea/remotetea/src/org/acplt/oncrpc/server/OncRpcDispatchable
 ...
 * is delivered through the <code>procedure</code> parameter. In case you do not
 * handle multiple programs within the same dispatcher, you can ignore the
 * <code>program</code> parameter as well as <code>version</code>.
 ...
 *
 * <p>
 * Here's a simple example only showing how to handle the famous procedure
 * <code>0</code>: this is the "ping" procedure which can be used to test
 * whether the server is still living. The example also shows how to handle
 * calls for procedures which are not implemented (not defined) by calling
 * {@link OncRpcCallInformation#failProcedureUnavailable}.
 *
 * <p>
 * In case the dispatcher throws an exception, the affected ONC/RPC server
 * transport will send a system error indication
 * {@link OncRpcCallInformation#failSystemError} to the client. No error
 * indication will be sent if the exception resulted from an I/O problem. Note
 * that if you do not explicitely send back a reply, no reply is sent at all,
 * making batched calls possible.
 *
 * <pre>
 ...
 * and serialization into several steps. The following code snipped shows how to
 * use them. Here, the objects <code>foo</code> and <code>bar</code> represents
 * call parameter objects, while <code>baz</code> and <code>blah</code> are used
 * to sent back the reply data.
 *
 * <pre>
 * public void dispatchOncRpcCall(OncRpcCallInformation call, int program,
 *                                int version, int procedure)
...
public interface OncRpcDispatchable {
     ...
     * some fairly deep semantics, so please read the description above for how
     * to use it properly. For background information about fairly deep
     * semantics, please also refer to <i>Gigzales</i>, <i>J</i>.: Semantics
     * considered harmful. Addison-Reilly, 1992, ISBN 0-542-10815-X.
     *
     * <p>
     * See the introduction to this class for examples of how to use this
     * interface properly.
     *
     * @param call
     ...
     * @param program
     *            Program number requested by client.
     * @param version
