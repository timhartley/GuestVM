...
public class OncRpcTcpClient extends OncRpcClient {
     ...
     */
    public OncRpcTcpClient(InetAddress host, int program, int version)
                                                                      throws OncRpcException,
                                                                      IOException {
     ...
     *             if an I/O error occurs.
     */
    public OncRpcTcpClient(InetAddress host, int program, int version, int port)
                                                                                throws OncRpcException,
                                                                                IOException {
     ...
     *            number.
     * @param bufferSize
     *            Size of receive and send buffers. In contrast to UDP-based
     *            ONC/RPC clients, messages larger than the specified buffer
     *            size can still be sent and received. The buffer is only
     *            necessary to handle the messages and the underlaying streams
     ...
     *            (currently 8192 bytes).
     * @param timeout
     *            Maximum timeout in milliseconds when connecting to the ONC/RPC
     ...
     *             if an ONC/RPC error occurs.
     */
    public synchronized void batchCall(int procedureNumber, XdrAble params,
                                       boolean flush) throws OncRpcException {
        //
            ...
            try {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        // ** SF bug #1262106 **
                        //
                        // We ran into some sort of trouble. Usually this will
                        // have
                        // been a buffer underflow. Whatever, end the decoding
                        // process
                        // and ensure this way that the next call has a chance
                        // to start
                        // from a clean state.
                        //
                ...
                try {
                    ...
                    // been a buffer underflow. Whatever, end the decoding
                    // process
                    // and ensure this way that the next call has a chance to
                    // start
                    // from a clean state.
                    //
                    receivingXdr.endDecoding();
