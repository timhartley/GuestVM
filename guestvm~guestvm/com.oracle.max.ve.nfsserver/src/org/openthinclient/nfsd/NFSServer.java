...
public class NFSServer extends NFSServerStub {
    private static final Logger logger = Logger.getLogger(NFSServer.class.getCanonicalName());
     ...
     * @param pathManager
     * @param port
     *            the port number to use. Specify '0', to use the default port
     * @param programNumber
     *            the rpc program number to use. Specify '0', to use the default
     *            program number
    ...
    /**
     * Validate the given name and make a file within the given parent
     * directory.
     *
     * @param params
     * @param dir
    ...
    private File makeFile(String name, File dir) {
        ...
        if (name.indexOf('/') >= 0 || name.indexOf('\\') >= 0) {
            logger.warning("Got fishy filename: " + name);
            return null;
        }
    ...
    protected diropres NFSPROC_CREATE_2(createargs params) {
        ...
        try {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("CREATE: " + fileToCreate);
            }

            if (fileToCreate.exists()) {
                /*
                 * This is corrent, but IRIX falls over with it.. possibly
                 * idempotence issue... ret.status = nfs_misc.NFSERR_EXIST;
                 * return ret;
                 */
            } else {
                // Create the file.
                try {
                    if (!fileToCreate.createNewFile()) {
                        if (logger.isLoggable(Level.INFO)) {
                            logger.info("CREATE: create failed for "
                                        + fileToCreate);
                        }
                        ret.status = nfsstat.NFSERR_IO;
                        return ret;
                    ...
                    logger.log(Level.WARNING, "CREATE: got exception for "
                                              + fileToCreate, e);
                    ret.status = nfsstat.NFSERR_ACCES;
                    return ret;
                } catch (final IOException e) {
                    if (logger.isLoggable(Level.INFO)) {
                        logger.log(Level.INFO, "CREATE: got exception for "
                                               + fileToCreate, e);
                    }
                    ret.status = nfsstat.NFSERR_IO;
                    return ret;
            ...
            logger.warning("CREATE: Got stale handle");
    ...
    protected attrstat NFSPROC_GETATTR_2(nfs_fh params) {
        ...
        try {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("GETATTR: " + path);
            }

            ret.attributes = path.getAttributes();
            // ret.status = nfsstat.NFS_OK;
            logger.fine("Returning attibutes for: " + path);
        } catch (final StaleHandleException e) {
            logger.warning("GETATTR: Got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
        } catch (final FileNotFoundException e) {
            ret.status = nfsstat.NFSERR_NOENT;
        }
    ...
    protected int NFSPROC_LINK_2(linkargs params) {
        ...
        try {
            ...
            logger.warning("LINK: Got stale handle");
            return nfsstat.NFSERR_STALE;
        }
        logger.warning("LINK: not supported: From: " + from + " To: "
    ...
    protected diropres NFSPROC_LOOKUP_2(diropargs params) {
        ...
        try {
            ...
            logger.warning("LOOKUP: Got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
        } catch (final FileNotFoundException e) {
            ret.status = nfsstat.NFSERR_NOENT;
        }
    ...
    protected diropres NFSPROC_MKDIR_2(createargs params) {
        ...
        try {
            ...
            try {
                ...
                logger.log(Level.WARNING, "MKDIR: got exception for "
                                          + dirToCreate, e);
                ret.status = nfsstat.NFSERR_ACCES;
                return ret;
            }
            ...
            logger.warning("MKDIR: Got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
        } catch (final FileNotFoundException e) {
            ret.status = nfsstat.NFSERR_NOENT;
        }
    ...
    protected readres NFSPROC_READ_2(readargs params) {
        ...
        try {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("READ: " + f + "," + count + " bytes " + offset
                            + " offset");
            }
            ...
            try {
                ...
                logger.log(Level.WARNING, "READ: got exception for " + f, e);
                ret.status = nfsstat.NFSERR_ACCES;
            } catch (final IOException e) {
                logger.log(Level.WARNING, "READ: got exception for " + f, e);
                ret.status = nfsstat.NFSERR_IO;
            ...
            logger.warning("READ: Got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
        } catch (final FileNotFoundException e) {
            if (logger.isLoggable(Level.INFO)) {
                logger.log(Level.INFO, "READ: the file has vanished.", e);
            }
            ret.status = nfsstat.NFSERR_NOENT;
        }
    ...
    protected readdirres orig_NFSPROC_READDIR_2(readdirargs params) {
        ...
        try {
            ...
            try {
                ...
                for (File f : dirlist) {
                    dirMapFileById.put(pathManager.getIDByFile(f), f);
                }

                // Prepare idMapNextByCurrent
                final int[] tmpArray = new int[dirlist.length];
                int n = 0;
                for (final int i : dirMapFileById.keySet()) {
                    tmpArray[n] = i;
                    n++;
                }

                // Create fileId -> nextFileId Map to use nextFileId as cookie.
                // The cookie must be a "persistent" pointer to the next entry
                // in the
                // directory
                ...
                for (Map.Entry<Integer, File> pairs : dirMapFileById.entrySet()) {
                    final entry next = new entry();

                    final File f = pairs.getValue();
                    String fileName = f.getName();
                    final Integer fileId = pairs.getKey();

                    // The client only want's file(s) from a certain cookie
                    // (fileId)
                    // upwards
                    if (fileId < cookie) {
                        continue;
                    ...
                    next.cookie = new nfscookie(
                                                new byte[nfs_prot.NFS_COOKIESIZE]);
                    // Set "pointer" to next directory entry
                    NFSServer.intToByte(idMapNextByCurrent.get(fileId),
                                        next.cookie.value);
                ...
                logger.log(Level.WARNING, "READDIR: got exception for " + dir,
                           e);
                ret.status = nfsstat.NFSERR_ACCES;
            }
        } catch (final StaleHandleException e1) {
            logger.warning("READDIR: Got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
        }
        return ret;
    };
    ...
    protected readdirres NFSPROC_READDIR_2(readdirargs params) {
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        readdirres ret;
        int cookie, startvalue, entrycounter;
        entry curr, next;
        String[] dirlist;
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        NFSFile dir;
        try {
            dir = pathManager.getNFSFileByHandle(params.dir);
        } catch (final StaleHandleException e1) {
            logger.warning("READDIR: Got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
            return ret;
        ...
        // nfs_misc.report("NFS readdir "+pathname);

        cookie = NFSServer.byteToInt(params.cookie.value, 0);
        ...
        // Get list of files
        dirlist = dir.getFile().list();

        if (logger.isLoggable(Level.FINE)) {
            logger.fine(dirlist.length + " files in directory");
            logger.fine("start cookie: " + startvalue);
        }

        for (int i = startvalue; i < dirlist.length; i++) {
            entrycounter++;

            File g = new File(dir.getFile(), dirlist[i]);
            if (!pathManager.createMissingHandles(g)) {
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("Cannot create missing handles for" + g);
                }
            }

            next = new entry();
            try {
                next.fileid = pathManager.getIDByFile(g);
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("listing: " + g);
                    logger.fine("fileid: " + next.fileid);
                }
            } catch (StaleHandleException e) {
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("stale handle for: " + g);
                }
                e.printStackTrace();
                continue;
            }
            ...
            next.name = new filename(new String(g.getName()));
            next.cookie = new nfscookie(new byte[nfs_prot.NFS_COOKIESIZE]);
            NFSServer.intToByte(cookie, next.cookie.value);
            ...
            // Rather than cock around trying to work out count
            // value, just return 40 entries at a time
            if (entrycounter > 200) {
                ret.reply.eof = false;
                return ret;
            }
        ...
        // nfs_misc.report("returning from readdir");
        return ret;
    }
    ...
    protected readlinkres NFSPROC_READLINK_2(nfs_fh params) {
        ...
        try {
            ...
            try {
                ...
                if (logger.isLoggable(Level.INFO)) {
                    logger.log(Level.INFO, "READ: got exception for " + f, e);
                }
                ret.status = nfsstat.NFSERR_IO;
            }
        } catch (final StaleHandleException e1) {
            logger.warning("READLINK: Got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
        }
    ...
    protected int NFSPROC_REMOVE_2(diropargs params) {
        ...
        try {
            ...
            try {
                ...
                if (!f.delete()) {
                    logger.warning("REMOVE: remove failed for " + f);
                    return nfsstat.NFSERR_IO;
                }
            } catch (final SecurityException e) {
                logger.log(Level.WARNING, "REMOVE: got exception for " + f, e);
                return nfsstat.NFSERR_ACCES;
            }

            pathManager.purgeFileAndHandle(f);
        } catch (final StaleHandleException e1) {
            logger.warning("REMOVE: Got stale handle");
            return nfsstat.NFSERR_STALE;
        }
                ...
                try {
                    ...
                    if (to.isFile() && to.exists()) {
                        if (!to.renameTo(renameTemp)) {
                            logger.warning("RENAME: rename failed for "
                                           + renameTemp);
                            return nfsstat.NFSERR_IO;
                        }
                    }
                    if (!from.renameTo(to)) {
                        logger.warning("RENAME: rename failed for " + from
                                       + " to " + to);
                        return nfsstat.NFSERR_IO;
                    }
                    ...
                    if (!to.equals(toCheckLink)) {
                        ...
                        if (!toCheckLink.delete()) {
                            logger.warning("RENAME: deleting failed for link "
                                           + toCheckLink);
                            return nfsstat.NFSERR_IO;
                        }
                    ...
                    if (renameTemp.isFile() && renameTemp.exists()) {
                        ...
                        if (!renameTemp.delete()) {
                            logger.warning("RENAME: deleting failed for "
                                           + renameTemp);
                        }
                    }
                }
            } catch (final StaleHandleException e1) {
                logger.warning("RENAME: got stale handle");
                return nfsstat.NFSERR_STALE;
            }
        }
    ...
    protected attrstat NFSPROC_SETATTR_2(sattrargs params) {
        ...
        try {
            ...
            try {
                if (f.getAttributes().size != params.attributes.size
                    ...
                    try {
                        ...
                        if (current < params.attributes.size) {
                            // can't truncate to larger size. emulate using
                            // dummy write.
                        ...
                        logger.log(Level.WARNING, "READ: got exception for "
                                                  + f, e);
                        ret.status = nfsstat.NFSERR_ACCES;
                    } catch (final IOException e) {
                        logger.log(Level.WARNING, "READ: got exception for "
                ...
                logger.log(Level.WARNING, "SETATTR: got exception for " + f, e);
                ret.status = nfsstat.NFSERR_ACCES;
                return ret;
            }
        } catch (final StaleHandleException e1) {
            logger.warning("SETATTR: got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
        } catch (final FileNotFoundException e) {
            ret.status = nfsstat.NFSERR_NOENT;
        }
    ...
    protected int NFSPROC_SYMLINK_2(symlinkargs params) {
        ...
        try {
            ...
            fromName = replaceColon(fromName, false);
            final File from = makeFile(fromName + SOFTLINK_TAG,
                                       fromdir.getFile());
            if (null == from) {
                return nfsstat.NFSERR_IO;
            }
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("SYMLINK: " + from + " to " + dest);
            }
            ...
            logger.warning("SYMLINK: got stale handle");
            return nfsstat.NFSERR_STALE;
        }
    ...
    protected attrstat NFSPROC_WRITE_2(writeargs params) {
        ...
        try {
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("WRITE: " + f + " of " + count + " bytes");
            }
            ...
            try {
                ...
                logger.log(Level.WARNING, "WRITE: got exception for " + f, e);
                ret.status = nfsstat.NFSERR_ACCES;
            } catch (final IOException e) {
                if (logger.isLoggable(Level.INFO)) {
                    logger.log(Level.WARNING, "WRITE: got exception for " + f,
                               e);
                }
                ret.status = nfsstat.NFSERR_IO;
            }
        } catch (final StaleHandleException e1) {
            logger.warning("WRITE: got stale handle");
            ret.status = nfsstat.NFSERR_STALE;
        } catch (final FileNotFoundException e) {
            ret.status = nfsstat.NFSERR_NOENT;
        }
    ...
    protected void NFSPROC_WRITECACHE_2() {
        logger.warning("WRITECACHE: not implemented");
    }
}
