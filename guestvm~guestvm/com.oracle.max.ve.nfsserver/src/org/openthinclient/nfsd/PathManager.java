...
public class PathManager {
    static final Logger logger = Logger.getLogger(PathManager.class.getCanonicalName());
    ...
    private static void parseHex(String line, int start, byte bytes[],
                                 int length) {
        int charIdx = start;
     ...
     */
    public PathManager(File handleDatabase, Exporter exporter)
                                                              throws IOException {
    ...
    public boolean createMissingHandles(File f) {
        if (!f.isAbsolute()) {
            f = f.getAbsoluteFile();
        }
        ...
        if (null == firstParent) {
            logger.severe("Unable to get any parent of: " + f);
            return false;
        }

        try {
            final NFSExport export = getNFSFileByHandle(
                                                        getHandleByFile(firstParent)).getExport();

            for (File fileToAdd : filesToAdd) {
                final nfs_fh fileToAddParentNfsHandle = getHandleByFile(fileToAdd.getParentFile());
                new NFSFile(getHandleByFile(fileToAdd), fileToAdd,
                            getNFSFileByHandle(fileToAddParentNfsHandle),
                            export);
            }
            return true;
    ...
    public void flushFile(File f) {
        try {
            ...
            logger.severe("Unable to flush cache for: " + f.getAbsolutePath());
        }
    }
    ...
    public synchronized void flushPathDatabase() throws IOException {
        if (null != handleDatabase && isChanged) {
            ...
            final File tmp = File.createTempFile("paths", ".db",
                                                 handleDatabase.getParentFile());
            if (null == tmp) {
                throw new IOException("Can't create tmp handle database at "
                                      + tmp);
            }
            ...
            for (NFSFile file : handlesToFiles.values()) {
                file.flushCache();
            ...
            final File handleDBBackup = new File(
                                                 handleDatabase.getAbsolutePath()
                                                         + "~");
     ...
     * @return
     * @throws StaleHandleException
     *             if the handle is not defined or the handle generation doesn't
     *             match.
     */
    public NFSFile getNFSFileByHandle(nfs_fh fh) throws StaleHandleException {
        ...
        if (!nfsFile.validateHandle(fh)) {
            throw new StaleHandleException(
                                           "Handle was defined, but wrong generation");
        }
    ...
    public synchronized void movePath(File from, File to) {
        ...
        for (Map.Entry<File, File> pairs : recursiveMoveMap.entrySet()) {
            moveMapEntries(pairs.getKey(), pairs.getValue());
        }
        isChanged = true;
    }
    ...
    public boolean removeFileFromNFS(Collection<File> filesList) {
        for (File file : filesList) {
            ...
            if (file.delete()) {
                ...
                logger.severe("Unable to remove File: " + file.getPath());
                return false;
            }
        }
        isChanged = true;
    ...
    /**
     * @throws Exception
     *             if something goes wrong. Yes, I don't want to be more
     *             specific here.
     */
    ...
    private nfs_fh createHandleForFile(File f, NFSExport export)
        ...
        if (null == export) {
            ...
            if (null == parentHandle) {
                throw new StaleHandleException(
                                               f
                                                       + " doesn't have a parent handle");
            }
            final int id = getIDFromHandle(parentHandle);
            final NFSFile parent = handlesToFiles.get(id);
            if (null == parent) {
                throw new StaleHandleException(
                                               "Not NFS file for parent handle for "
                                                       + f);
            }

            handlesToFiles.put(h,
                               new NFSFile(fh, f, parent, parent.getExport()));
        } else {
    ...
    private synchronized LinkedHashMap<File, File> getMoveMap(
                                                              File from,
                                                              File to,
        ...
        if (to.isDirectory()) {
            ...
            for (File f : filesToHandles.keySet()) {
    ...
    private void loadPathDatabase() throws IOException {
        if (null != handleDatabase) {
            ...
            final File tmp = File.createTempFile(
                                                 "paths",
                                                 ".db",
                                                 handleDatabase.getAbsoluteFile().getParentFile());
            if (null == tmp) {
                throw new IOException("Can't create tmp handle database at "
                                      + tmp);
            }
            ...
            if (handleDatabase.exists()) {
                    ...
                    try {
                        ...
                        if (!(path.exists() || path.isHidden())) {
                            if (logger.isLoggable(Level.INFO)) {
                                logger.info("Not loading nonexistent path "
                                            + path);
                            }
                            continue;
                        }
                        ...
                        // if several exports match the path, the one with the
                        // best
                        // (longest)
                        // match is chosen.
                        ...
                        if (null == bestMatch) {
                            if (logger.isLoggable(Level.INFO)) {
                                logger.info("Path seems to be no longer exported: "
                                            + path);
                            }
                            continue;
                        }
                        ...
                        currentHandleCounter = Math.max(currentHandleCounter,
                                                        id + 1);
                        handlesToFiles.put(id, new NFSFile(fh, path, null,
                                                           bestMatch));
                        filesToHandles.put(path, fh);
                    } catch (final Exception e) {
                        // ignore the line when parsing failed.
                        logger.warning("Can't parse this line: " + line);
                    }
                }
                ...
                for (NFSFile file : handlesToFiles.values()) {
                    ...
                    if (null == parentHandle
                        ...
                        logger.warning("Parent for file " + file.getFile()
                        ...
                        if (null == parentFile) {
                            logger.warning("Parent file for handle not found. Should not happen!");
                            filesToRemove.add(file);
                        } else {
    ...
    private synchronized void moveMapEntries(File from, File to) {
        ...
        if (null != fhFrom) {
            ...
            if (null != parentHandleTo) {
                ...
                if (null != parentTo) {
                    final NFSFile nfsFileTo = new NFSFile(
                                                          nfsFileFrom.getHandle(),
                                                          to, parentTo,
                                                          parentTo.getExport());
                    filesToHandles.put(to, fhFrom);
                    handlesToFiles.put(handleToInt(fhFrom), nfsFileTo);
                } else {
                    logger.warning("missing handle->file map for parentHandleTo: "
                                   + parentHandleTo);
                }
            } else {
                logger.warning("missing file->handle map for parentFileTo: "
                               + parentFileTo);
            }

            isChanged = true;
        } else {
            logger.warning("missing file->handle map for from: " + from);
        }
    }
