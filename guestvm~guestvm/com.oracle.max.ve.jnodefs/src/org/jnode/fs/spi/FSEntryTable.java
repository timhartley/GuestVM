 ...
 */
/*
 * $Id: FSEntryTable.java 4975 2009-02-02 08:30:52Z lsantha $
 *
...
public class FSEntryTable extends AbstractFSObject {
    ...
    private static final Logger log = Logger.getLogger(FSEntryTable.class.getName());
        ...
        for (FSEntry entry : entryList) {
            if (entry == null) {
                ...
                if (log.isLoggable(Level.FINEST)) {
                    log.log(Level.FINEST, "FSEntryTable: adding entry " + name + " (length=+"
                        + name.length() + ")");
                }
    ...
    public int rename(String oldName, String newName) {
        if (log.isLoggable(Level.FINEST)) {
            log.log(Level.FINEST, "<<< BEGIN rename oldName=" + oldName + " newName=" + newName
                + " >>>");
        }
        if (log.isLoggable(Level.FINEST)) {
           log.log(Level.FINEST, "rename: table=" + toString());
        }
        oldName = normalizeName(oldName);
        newName = normalizeName(newName);
        if (log.isLoggable(Level.FINEST)) {
           log.log(Level.FINEST, "rename oldName=" + oldName + " newName=" + newName);
        }
        if (!entryNames.contains(oldName)) {
            if (log.isLoggable(Level.FINEST)) {
                 log.log(Level.FINEST, "<<< END rename return false (oldName not found) >>>");
            }
            return -1;
        }
        ...
        if (log.isLoggable(Level.FINEST)) {
           log.log(Level.FINEST, "<<< END rename return true >>>");
        }
        return index;
    }
    ...
    public int setFreeEntry(FSEntry newEntry) throws IOException {
        ...
        if (index < 0) {
            if (log.isLoggable(Level.FINEST)) {
                log.log(Level.FINEST, "setFreeEntry: ERROR: entry table is full");
            }
